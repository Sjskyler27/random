...
class Bot:
    def __init__(self, name, position=0, timetocorner=0, currentcorner=0, speedofcorner=0):
        self.name = name
    ...
    def move(self):
        ...
        if(self.doublecorner):
            ...
            #but how
            #maybe position+= ttc+1 that gets it past the corner then call get map values so you have the new ttc then do a default move. 
        else:
...
class Race:
    ...
    def start_race(self):
        ...
        for i in range(0,self.times):
            ...
            self.worksheet.cell(row=num_rows+1, column=6, value=value)
...
if debug:
    ...
    times = 20
for i in range(3,4):
    race = Race('./HEATMAPTEST/RaceMaps.xlsx', f'Sheet{i}',times)
